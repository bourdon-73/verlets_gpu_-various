...
const float BASE_RESPONSE_COEF = 0.20; // Base response coefficient
...
void applyConstraint(uint id, float radius, vec2 c_center, float c_radius, float dt_sub) {
    uint base = id * FLOATS_PER_PARTICLE;
    ...
    // Apply constraint with gradual correction for better stability
    float tolerance = max(1e-6, (constraint_radius - radius) * 0.01);
    if (dist > (constraint_radius - radius) - EPSILON) {
        vec2 n = v / dist;
        
        // Calculate how far we need to move to satisfy the constraint
        float overlap = dist - (constraint_radius - radius);
        
        // Apply scaled constraint based on substeps
        // Use a smoothed correction to avoid jumps
        float correction_factor = min(1.0, 5.0 * dt_sub);
        vec2 correction = n * overlap * correction_factor;
        
        vec2 new_pos = pos + correction;
        read_particles_buffer.data[base + 0] = new_pos.x;
        read_particles_buffer.data[base + 1] = new_pos.y;
    }
}

void CheckCollision(uint index, uint id2, float dt_sub) {
    ...
    // Fix: Get the correct last position for particle 2
    vec2 lastpos2 = vec2(read_particles_buffer.data[base2 + 2], read_particles_buffer.data[base2 + 3]);

    vec2 v = pos1 - pos2;
    float dist2 = v.x * v.x + v.y * v.y;

    float radius = 8.0;//read_particles_buffer.data[base+6];
    float kradius = 8.0;//read_particles_buffer.data[base2+6];
    float min_dist = radius + kradius;
    
    // Scale response coefficient by dt_sub to maintain consistent behavior across substeps
    // We divide by SUBSTEPS to ensure total response over all substeps equals BASE_RESPONSE_COEF
    float scaled_response_coef = BASE_RESPONSE_COEF / float(SUBSTEPS);
    ...
    if (dist2 < (min_dist * min_dist) - tolerance) {
        ...
        // Scale the collision response to account for substeps
        float delta = 0.5 * scaled_response_coef * (dist - min_dist);
...
void CollisionDetection(uint id, float dt_sub) {
    ...
    for (uint i = 0; i < (N - 1) - id; i++) {
        ...
        CheckCollision(id, id + i + 1, dt_sub);
    }
}
...
void main() {
    ...
    // Apply physics in substeps
    for (int i = 0; i < SUBSTEPS; i++) {
        process_base_forces(id, dt_sub);
        CollisionDetection(id, dt_sub);
        applyConstraint(id, 8, m_constraint_center, constraint_radius, dt_sub);
    }

    // Write final position to image
    vec4 data = Write_to_image(id);
