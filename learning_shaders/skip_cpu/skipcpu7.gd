...
@export var multimesh_instance: MultiMeshInstance2D #= $MultiMeshInstance2D
...
func _ready():
	...
	var shader_file = load("res://skip_cpu/image_compute2.glsl")
	...
	img = Image.create_from_data(texture_width, texture_height, false, Image.FORMAT_RGBAF, data)
...
# Later, no update needed — shader sees live data.
	...
	var camera_pos = my_camera.global_position
	matt.shader = preload("res://skip_cpu/verletshader.gdshader")
	matt.set_shader_parameter("particles_data", tex)
	matt.set_shader_parameter("camera_position", camera_pos)
